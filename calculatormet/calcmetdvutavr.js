...
(function($) {
    Drupal.behaviors.calcmetDvutavr = {
        'attach': function(context, settings) {
            ...
            var name = $('select#edit-dvutavr-options');
            //console.log(diametr);
            var url = Drupal.settings.basePath + 'services/calcmet/dvutavr/dvutavr_change_info';
            var dlina = $('input#edit-dvutavr-length');
            ...
            name.on('change', function(){                
                $('select#edit-dvutavr-options option:selected').each(
                    function(){
                        diamsel = $(this).val();
                        console.log(diamsel);
                        $.post(url, {'dvutavrselect' : diamsel}, function(data){
                            height.text(data.dvutavrheight);
                            width.text(data.dvutavrwidth);
                            thickness.text(data.dvutavrthickness);
                            polkathickness.text(data.dvutavrthicknesspolka);
                            csa.text(data.csa);
                            mass.text(data.massonem);
                            drawonemetr = data.drawonemetr;
                            if(! parseFloat(dlina.val()) )
                                //console.log(typeof(dlina.val()));
                                return;
                            $resultmassa = Math.round($(this).val()*mass.text()).toFixed(2);
                            $resultcsadraw = Math.round($(this).val()*drawonemetr).toFixed(2);
        ...
        btntospec.on('click', function(e){
            ...
            var dintbl = $('select#edit-dvutavr-options option:selected').text();
